...
And in your staging-values.yml have autoscaling is true so in deployment repicas count will be ignored and HPA will be created instead so if you want to checked template output you have to set autoscaling to false in staging-values.yaml and then check the output of replicas count.
...
I have forwarded the service port to 8082 on localhost for testing:
